...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${w} ${h}" width="${wMm}mm" height="${hMm}mm">
  <path d="${d}" stroke="black" stroke-width="1" fill="none" stroke-linejoin="round" stroke-linecap="round"/>
    ...
    /* ================================================================
       INTERACTIVE CROP & ROTATION
       Additive feature, no existing code modified.
    ================================================================ */
    (function initCropRotate() {
        const modal    = $('cropRotateModal');
        const btnOpen  = $('btnCropRotate');
        const btnClose = $('btnCloseCropRotate');
        const crCanvas = $('crCanvas');
        const crOverlay= $('crOverlay');
        if (!modal||!btnOpen||!crCanvas) return;

        let crState = { rotation:0, flipH:false, flipV:false,
                        crop:{x:0,y:0,w:1,h:1}, dragging:false,
                        handle:null, startMx:0, startMy:0, startCrop:null };
        let crImage=null, crScale=1;

        const crCtx = crCanvas.getContext('2d');
        const crOCtx= crOverlay.getContext('2d');
        const crRot = $('crRotation');
        const crRotN= $('crRotNum');

        crRot.addEventListener('input',()=>{ crRotN.value=crRot.value; crState.rotation=parseFloat(crRot.value); renderCrop(); });
        crRotN.addEventListener('input',()=>{ crRot.value=crRotN.value; crState.rotation=parseFloat(crRotN.value); renderCrop(); });

        function openModal() {
            if (!state.image) { toast('Import an image first'); return; }
            crImage=state.image;
            crState.rotation=0; crState.flipH=false; crState.flipV=false;
            crState.crop={x:0,y:0,w:crImage.width,h:crImage.height};
            crRot.value=crRotN.value=0;
            modal.classList.add('open'); modal.setAttribute('aria-hidden','false');
        ...
        function getHandle(mx, my) {
            ...
            const cx=c.x*sc,cy=c.y*sc,cw=c.w*sc,ch=c.h*sc;
            const tol=12;
            const handles=[['tl',cx,cy],['tr',cx+cw,cy],['bl',cx,cy+ch],['br',cx+cw,cy+ch]];
            for(const[n,hx,hy]of handles){
                if(Math.abs(mx-hx)<tol&&Math.abs(my-hy)<tol) return n;
            }
        ...
        window.addEventListener('mousemove',e=>{
            ...
            let {x,y,w,h}=sc;
            const minSz=10;
            if(crState.handle==='tl'){ x=clamp(sc.x+dx,0,sc.x+sc.w-minSz); y=clamp(sc.y+dy,0,sc.y+sc.h-minSz); w=sc.x+sc.w-x; h=sc.y+sc.h-y; }
            else if(crState.handle==='tr'){ w=clamp(sc.w+dx,minSz,nat.width-sc.x); y=clamp(sc.y+dy,0,sc.y+sc.h-minSz); h=sc.y+sc.h-y; }
            else if(crState.handle==='bl'){ x=clamp(sc.x+dx,0,sc.x+sc.w-minSz); h=clamp(sc.h+dy,minSz,nat.height-sc.y); w=sc.x+sc.w-x; }
            else if(crState.handle==='br'){ w=clamp(sc.w+dx,minSz,nat.width-sc.x); h=clamp(sc.h+dy,minSz,nat.height-sc.y); }
        ...
        $('btnCrRot180').addEventListener('click',()=>{ crState.rotation=(crState.rotation+180)%360; crRot.value=crRotN.value=crState.rotation; renderCrop(); });
        $('btnCrFlipH').addEventListener('click',()=>{ crState.flipH=!crState.flipH; renderCrop(); });
        $('btnCrFlipV').addEventListener('click',()=>{ crState.flipV=!crState.flipV; renderCrop(); });
        $('btnCrReset').addEventListener('click',()=>{
            if(!crImage) return;
            crState.rotation=0; crState.flipH=false; crState.flipV=false;
            crState.crop={x:0,y:0,w:crImage.width,h:crImage.height};
            crRot.value=crRotN.value=0; renderCrop();
        });

        $('btnCrApply').addEventListener('click',()=>{
            if(!crImage) return;
            const {x,y,w,h}=crState.crop;
            const rot=crState.rotation, fH=crState.flipH, fV=crState.flipV;

            /* Step 1: crop the original image first (before any rotation/flip).
               The crop handles are defined in original image pixel space, so we
               must extract that region from the untransformed source — not from
               a rotated canvas where the coordinates no longer correspond. */
            const cropW=Math.max(1,Math.min(w, crImage.width-x));
            const cropH=Math.max(1,Math.min(h, crImage.height-y));
            const cropC=document.createElement('canvas');
            cropC.width=cropW; cropC.height=cropH;
            cropC.getContext('2d').drawImage(crImage, x, y, cropW, cropH, 0, 0, cropW, cropH);

            /* Step 2: rotate + flip the already-cropped region onto a diagonal-
               sized canvas so rotated corners are never clipped. */
            const diag=Math.ceil(Math.sqrt(cropW*cropW+cropH*cropH));
            const fullC=document.createElement('canvas');
            fullC.width=diag; fullC.height=diag;
            const fCtx=fullC.getContext('2d');
            fCtx.save();
            fCtx.translate(diag/2, diag/2);
            fCtx.rotate(rot*Math.PI/180);
            fCtx.scale(fH?-1:1, fV?-1:1);
            fCtx.drawImage(cropC, -cropW/2, -cropH/2);
            fCtx.restore();

            /* Step 3: trim diagonal canvas to the exact rotated bounding box
               so no transparent padding is carried into the pipeline. */
            const rad=rot*Math.PI/180;
            const outW=Math.round(Math.abs(cropW*Math.cos(rad))+Math.abs(cropH*Math.sin(rad)));
            const outH=Math.round(Math.abs(cropW*Math.sin(rad))+Math.abs(cropH*Math.cos(rad)));
            const trimC=document.createElement('canvas');
            trimC.width=outW; trimC.height=outH;
            trimC.getContext('2d').drawImage(fullC,
                Math.round((diag-outW)/2), Math.round((diag-outH)/2),
                outW, outH, 0, 0, outW, outH);

            /* Step 4: load result as new image and re-process */
            trimC.toBlob(blob=>{
                const url=URL.createObjectURL(blob);
                const img=new Image();
                img.onload=()=>{
                    ...
                    const dpi=parseFloat($('imgDpiNum').value)||300;
                    $('imgWidth').value=Math.round((img.width/dpi)*25.4);
                    $('imgHeight').value=Math.round((img.height/dpi)*25.4);
                    closeModal();
                    toast('Crop & rotate applied  -  re-processing...');
                    queueProcess();
                };
                img.src=url;
            },'image/png');
    ...
    (function extendAria() {
        const extra=[
            ...
            ['btnCropRotate',  {'aria-label':'Open interactive crop and rotate tool'}],
