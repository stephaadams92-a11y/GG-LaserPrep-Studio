/**
 * GG LaserPrep v7  -  Main Application Module
 *
 * Changes from v6:
...
(function () {
    ...
    */
    ...
    /* FIX: If every stored section is collapsed the user can't open anything.
       ...
       Guard: if ALL keys are true (collapsed), wipe and start fresh so sections
       default to open on load. */
    ...
    ['chkProtectHigh','chkHistEq','chkInvert','chkSimulate','chkLanczos','chkCatmullRom'].forEach(id => {
        const el = $(id); if (el) el.addEventListener('change', queueProcess);
    });
  ...
  <path d="${d}" stroke="black" stroke-width="1" fill="none" stroke-linejoin="round" stroke-linecap="round"/>
    ...
    (function initCropRotate() {
        ...
        $('btnCrApply').addEventListener('click',()=>{
            ...
            /* Step 1: render the full image with rotation + flip onto a diagonal-sized
               canvas so rotated corners are never clipped (FIX: was same-size as source) */
            const diag=Math.ceil(Math.sqrt(crImage.width*crImage.width+crImage.height*crImage.height));
            ...
            fCtx.drawImage(crImage, -crImage.width/2, -crImage.height/2);
            fCtx.restore();

            /* Offset into the diagonal canvas where the image origin sits */
            const offX=Math.round((diag-crImage.width)/2);
            const offY=Math.round((diag-crImage.height)/2);

            /* Step 2: crop the desired region from the transformed canvas */
            ...
            tmpC.getContext('2d').drawImage(fullC, offX+x, offY+y, cropW, cropH, 0, 0, cropW, cropH);

            /* Step 3: load result as new image and re-process */
            tmpC.toBlob(blob=>{
                const url=URL.createObjectURL(blob);
                const img=new Image();
                img.onload=()=>{
